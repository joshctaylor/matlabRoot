    ...
    if (shimmer.start)                                                     % TRUE if the shimmer starts streaming
        ...
        while (elapsedTime < captureDuration)            
            ...
            if (firsttime==true && isempty(newData)~=1) 
                ...
                signalNamesString=[char('Time Stamp'), char(9), char('ECG LL-RA'), char(9), char('ECG LL-RA Filtered'), char(9), char('Heart Rate')]; % create a single string, signalNamesString    
