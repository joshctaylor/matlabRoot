function shimmer = plotandwriteecgleadoffdetectionexample(comPort, captureDuration, fileName)
    %PLOTANDWRITEECGLEADOFFDETECTIONEXAMPLE - Plotting ecg signal and lead-off detection bytes
    % and write to file
    ...
    if (shimmer.connect)
        ...
        if (shimmer.start)                                                     % TRUE if the shimmer starts streaming 
            ...
            set(h.figure1, 'Position', [100, 600, 800, 400]);
            h.figure2=figure('Name','Shimmer ECG signals');                    % Create a handle to figure for plotting data from shimmer
            set(h.figure2, 'Position', [950, 600, 800, 400]);
            h.figure3=figure('Name','Shimmer Lead-off detection signals');     % Create a handle to figure for plotting data from shimmer
            set(h.figure3, 'Position', [100, 100, 800, 900]);
            h.figure4=figure('Name','Shimmer Lead-off detection signals');     % Create a handle to figure for plotting data from shimmer
            set(h.figure4, 'Position', [950, 100, 800, 400]);
            ...
            while (elapsedTime < captureDuration)       
                ...
                if ~isempty(newData)                                           % TRUE if new data has arrived
                    ...
                    chIndex(5) = find(ismember(signalNameArray, 'Lead-off ECG LL'));
                    chIndex(6) = find(ismember(signalNameArray, 'Lead-off ECG LA'));
                    chIndex(7) = find(ismember(signalNameArray, 'Lead-off ECG RA'));
                    chIndex(8) = find(ismember(signalNameArray,'Lead-off ECG RLD'));
                    chIndex(9) = find(ismember(signalNameArray,'Lead-off ECG Vx'));
                    
                    chIndex(10) = find(ismember(signalNameArray,'EXG1 STA'));
                    chIndex(11) = find(ismember(signalNameArray,'EXG2 STA'));
                    ...
                    signalIndex = chIndex(10);
                    plot(sampleNumber,plotData(:,10));                     % Plot 'EXG1 STA'
                    legendName1=[signalFormatArray{signalIndex} ' ' signalNameArray{signalIndex} ' (' signalUnitArray{signalIndex} ')'];
                    ...
                    plot(sampleNumber,plotData(:,5));                      % Plot 'Lead-off ECG LL'
                    legendName1=[signalFormatArray{signalIndex} ' ' signalNameArray{signalIndex} ' (' signalUnitArray{signalIndex} ')'];  
                    legend(legendName1, 'Location','West');                % Add legend to plot
                    xlim([sampleNumber(1) sampleNumber(end)]);
                    ylim([-1 1]);
                    ...
                    plot(sampleNumber,plotData(:,6));                      % Plot 'Lead-off ECG LA'
                    legendName1=[signalFormatArray{signalIndex} ' ' signalNameArray{signalIndex} ' (' signalUnitArray{signalIndex} ')'];  
                    legend(legendName1, 'Location','West');                % Add legend to plot
                    xlim([sampleNumber(1) sampleNumber(end)]);
                    ylim([-1 1]);
                    ...
                    plot(sampleNumber,plotData(:,7));                      % Plot 'Lead-off ECG RA'
                    legendName1=[signalFormatArray{signalIndex} ' ' signalNameArray{signalIndex} ' (' signalUnitArray{signalIndex} ')'];  
                    legend(legendName1, 'Location','West');                % Add legend to plot
                    xlim([sampleNumber(1) sampleNumber(end)]);
                    ylim([-1 1]);
                    ...
                    plot(sampleNumber,plotData(:,8));                      % Plot 'Lead-off ECG RLD'
                    legendName1=[signalFormatArray{signalIndex} ' ' signalNameArray{signalIndex} ' (' signalUnitArray{signalIndex} ')'];  
                    legend(legendName1, 'Location','West');                % Add legend to plot
                    xlim([sampleNumber(1) sampleNumber(end)]);
                    ylim([-1 1]);
                    
                    set(0,'CurrentFigure',h.figure4);
                    
                    subplot(2,1,1);                                        % Create subplot
                    signalIndex = chIndex(11);
                    plot(sampleNumber,plotData(:,11));                     % Plot 'EXG2 STA'
                    legendName1=[signalFormatArray{signalIndex} ' ' signalNameArray{signalIndex} ' (' signalUnitArray{signalIndex} ')'];
                    legend(legendName1, 'Location','West');                % Add legend to plot
                    xlim([sampleNumber(1) sampleNumber(end)]);
                    ...
                    plot(sampleNumber,plotData(:,9));                      % Plot 'Lead-off ECG Vx'
                    legendName1=[signalFormatArray{signalIndex} ' ' signalNameArray{signalIndex} ' (' signalUnitArray{signalIndex} ')'];
                    legend(legendName1, 'Location','West');                % Add legend to plot
                    xlim([sampleNumber(1) sampleNumber(end)]);
                    ylim([-1 1]);
