...
% Modified TPG 7/7/2004 to add show data statistics for time histories
% Modified TPG 21/7/2004 to replace 'samples per second' with 'sample
% increment'
...
if exist('defineflag');
    ...
    switch linedata.dtype
        case 1
            ...
            fi=num2str(xvals(2)-xvals(1));
            uimenu(cmenu, 'Label',[num2str(length(xvals)),' samples'], 'Enable','Off');
            uimenu(cmenu, 'Label',[fi,' sample increment'], 'Enable','Off');
            uimenu(cmenu, 'Label',['Mean: ',num2str(mean(yvals)),'; median: ',num2str(median(yvals)),'; r.m.s: ',num2str(std(yvals))], 'Enable','Off');  
    uimenu(cmenu, 'Label',['Min: ',num2str(min(yvals)),'; max: ',num2str(max(yvals)),'; range: ',num2str(abs(max(yvals)-min(yvals)))], 'Enable','Off'); 
        case 2
